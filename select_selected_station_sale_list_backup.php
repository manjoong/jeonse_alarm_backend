...
//////////////////////////////////////////////////////////////
    ...
    /* select data */
    // 1. 실행할 쿼리문을 작성합니다.
    $SaleListSQL = "SELECT * FROM sale WHERE (station, type) IN (SELECT station_id, type FROM selected_station WHERE user_id = '$_GET[id]') ORDER BY create_dt DESC";
        ...
        while ($row=mysqli_fetch_array($result_sale_list)) {
            foreach ($selected_station_json_decode as $row_selecte_station){
                if ($row_selecte_station['type'] == $row[6] && $row_selecte_station['station_id'] == $row[7] && strtotime($row_selecte_station['created_dt']) <= strtotime($row[8])){
                    $count = $count + 1;
                    array_push($data,array('id'=>$row[0],'name'=>$row[1],'naver_id'=>$row[2],'price'=>$row[3],'img_link'=>$row[4],'naver_link'=>$row[5],'type'=>$row[6],'station'=>$row[7],'create_dt'=>$row[8], 'count'=>$count));
                }
            }
        }
